...
3. **MCP Servers** (`backend/src/mcp/servers`) — one per domain (Patient,
   Doctor, Appointment, Medical Record, Laboratory, Prescription, Pharmacy,
   Billing, Inventory, Admission, Analytics, Notification). Each server
   exposes a small set of read-oriented tools/resources scoped to its own
   collections and enforces the requesting user's permissions before
   returning any data.
