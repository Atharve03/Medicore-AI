...
{ _id, email, passwordHash, role: enum[admin,doctor,patient,receptionist,
  nurse,pharmacist,labTechnician], isActive, isEmailVerified,
  otp: { codeHash, purpose: enum[registration,login], expiresAt, attempts },
  lastLoginAt, createdAt, updatedAt }
```
`otp` holds at most one active code at a time (a SHA-256 hash, never the
raw code); requesting a new one overwrites it. Added in the OTP/SMTP
verification update: public self-registration and every login now require
completing an emailed 6-digit code before tokens are issued. Admin-created
accounts (Admin module) are marked `isEmailVerified: true` immediately —
an admin directly provisioning an account is a different trust model than
public self-registration — but still require a login OTP on first sign-in.
