...
async function assertCanMutateAppointment(appointment, requestingUser, action) {
  if (!requestingUser) throw ApiError.unauthorized('Authentication required');

  if (action === 'status') {
    if (requestingUser.role === 'receptionist') return;
    if (requestingUser.role !== 'doctor') throw ApiError.forbidden('Not permitted to update this appointment');
    const doctor = await doctorRepository.findByUserId(requestingUser.id);
    if (!doctor || String(doctor._id) !== String(appointment.doctorId)) {
      throw ApiError.forbidden('A doctor may update only their assigned appointments');
    }
    return;
  }

  if (requestingUser.role === 'receptionist') return;
  if (requestingUser.role !== 'patient') throw ApiError.forbidden('Not permitted to cancel this appointment');
  const patient = await patientRepository.findByUserId(requestingUser.id);
  if (!patient || String(patient._id) !== String(appointment.patientId)) {
    throw ApiError.forbidden('A patient may cancel only their own appointments');
  }
}

async function updateStatus(id, nextStatus, requestingUser) {
  const appointment = await appointmentRepository.findById(id);
  if (!appointment) {
    throw ApiError.notFound('Appointment not found');
  }

  await assertCanMutateAppointment(appointment, requestingUser, 'status');
 ...
 */
async function cancelAppointment(id, requestingUser) {
  const appointment = await appointmentRepository.findById(id);
  if (!appointment) throw ApiError.notFound('Appointment not found');
  await assertCanMutateAppointment(appointment, requestingUser, 'cancel');

  const allowed = ALLOWED_TRANSITIONS[appointment.status] || [];
  if (!allowed.includes('cancelled')) {
    throw ApiError.badRequest(`Cannot transition appointment from '${appointment.status}' to 'cancelled'`);
  }
